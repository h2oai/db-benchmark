...
export SPARK_MASTER_IP="mr-0xd6"
...
cmdall="rm -rf ~/tmp/spark; export SPARK_HOME='$SPARK_HOME'; export SPARK_MASTER='$SPARK_MASTER'; $SPARK_HOME/sbin/start-slave.sh $SPARK_MASTER"
for i in $SPARK_WORKER_IP; do cmd="ssh $USER@$i \""$cmdall"\""; echo $cmd; eval $cmd; done
sleep 15

# execute scala benchmark script
$SPARK_HOME/bin/spark-shell --master $SPARK_MASTER --packages com.databricks:spark-csv_2.10:1.3.0 --num-executors 9 --executor-memory 24G --driver-memory 24G --conf spark.driver.maxResultSize=0 -i join-spark.scala
sleep 5
