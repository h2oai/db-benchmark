...
# start cluster
for i in $CLUSTER; do (ssh $USER@$i "nohup java $MEM -XX:-PrintGCDetails -cp h2o.jar water.H2OApp -name $H2O_NAME -port $H2O_PORT 2>&1 >> ~/cluster.log" & ); done && sleep 2 && echo Started h2o cluster ok on $CLUSTER
sleep 15
