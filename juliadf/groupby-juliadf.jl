...
range_fun(x) = combine(groupby(x, :id3), [:v1, :v2] => ((v1, v2) -> maximum(v1)-minimum(v2))) => :range_v1_v2);
GC.gc();
t = @elapsed(ANS = range_fun(x)); println(size(ANS)); flush(stdout);
m = memory_usage();
...
largest_fun(x) = combine(groupby(x, :id6), :v3 => (x -> partialsort(x, 1:min(2, length(x)), rev=true)) => :largest2_v3);
GC.gc();
t = @elapsed(ANS = largest_fun(x)); println(size(ANS)); flush(stdout);
m = memory_usage();
...
cor_fun(x) = combine(groupby(x, [:id2, :id4]), [:v1, :v2] => ((v1, v2) -> cor(v1, v2)^2) => :r2);
GC.gc();
t = @elapsed(ANS = cor_fun(x)); println(size(ANS)); flush(stdout);
m = memory_usage();
