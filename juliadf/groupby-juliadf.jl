...
#GC.gc();
#t = @elapsed (ANS = by(x, [:id2, :id4], x->(range_v1_v2=max(x.v1)-min(x.v2))); println(size(ANS)); flush(stdout));
#m = memory_usage();
...
GC.gc();
t = @elapsed (ANS = by(sort(x[[:id2, :id4, :v3]], :v3, rev=true), [:id2, :id4], x->(largest2_v3=first(x[[:v3]], 2).v3)); println(size(ANS)); flush(stdout)); # TODO rename x1 to largest2_v3
m = memory_usage();
chkt = @elapsed chk = sum(ANS.x1); # TODO rename x1 to largest2_v3
write_log(1, task, data_name, in_rows, question, size(ANS, 1), size(ANS, 2), solution, ver, git, fun, t, m, cache, make_chk(chk), chkt);
ANS = 0;
GC.gc();
t = @elapsed (ANS = by(sort(x[[:id2, :id4, :v3]], :v3, rev=true), [:id2, :id4], x->(largest2_v3=first(x[[:v3]], 2).v3)); println(size(ANS)); flush(stdout));
m = memory_usage();
chkt = @elapsed chk = sum(ANS.x1);
write_log(2, task, data_name, in_rows, question, size(ANS, 1), size(ANS, 2), solution, ver, git, fun, t, m, cache, make_chk(chk), chkt);
println(first(ANS, 3));
println(last(ANS, 3));
ANS = 0;
...
GC.gc();
t = @elapsed (ANS = by(x, [:id2, :id4], x->(r2=cor(x.v1, x.v2)^2)); println(size(ANS)); flush(stdout)); # TODO rename x1 to r2
m = memory_usage();
chkt = @elapsed chk = sum(ANS.x1); # TODO rename x1 to r2
write_log(1, task, data_name, in_rows, question, size(ANS, 1), size(ANS, 2), solution, ver, git, fun, t, m, cache, make_chk(chk), chkt);
ANS = 0;
GC.gc();
t = @elapsed (ANS = by(x, [:id2, :id4], x->(r2=cor(x.v1, x.v2)^2)); println(size(ANS)); flush(stdout));
m = memory_usage();
chkt = @elapsed chk = sum(ANS.x1);
write_log(2, task, data_name, in_rows, question, size(ANS, 1), size(ANS, 2), solution, ver, git, fun, t, m, cache, make_chk(chk), chkt);
println(first(ANS, 3));
