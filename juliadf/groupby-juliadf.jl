...
range_fun(v1, v2) = maximum(v1)-minimum(v2))
GC.gc();
t = @elapsed (ANS = combine(groupby(x, :id3), [:v1, :v2] => range_fun => :range_v1_v2); println(size(ANS)); flush(stdout));
m = memory_usage();
...
largest_fun(x) = partialsort(x, 1:min(2, length(x)), rev=true)
GC.gc();
t = @elapsed (ANS = combine(groupby(x, :id6), :v3 => largest_fun => :largest2_v3); println(size(ANS)); flush(stdout));
m = memory_usage();
...
cor_fun(v1, v2) = cor(v1, v2)^2
GC.gc();
t = @elapsed (ANS = combine(groupby(x, [:id2, :id4]), [:v1, :v2] => cor_fun => :r2); println(size(ANS)); flush(stdout));
m = memory_usage();
