...
spark.conf.set("spark.executor.memory", "100g")
...
ans = spark.sql("select * from x limit 3").collect()
# debug below
exit(0)

#ans = spark.sql("select sum(v1) as v1 from x group by id1").collect()
#ans = x.groupby(['id1']).agg({'v1':'sum'})
print_shape_spark(ans)
