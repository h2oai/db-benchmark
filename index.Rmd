...
This page aims to benchmark various database-like tools popular in open-source data science. It runs regularly against very latest versions of these packages and automatically updates. We provide this as a service to both developers of these packages and to users. You can find out more about the project in [the talk made by Matt Dowle on H2OWorld 2019 NYC conference](https://www.youtube.com/watch?v=fZpA_cU0SPg).
...
link = function(data_name, q_group, report_name) {
  ...
  paste(sprintf("[%s](%s)", q_group, file.path(path, fnam)), collapse=", ")
}
...
Timings are presented for a single dataset case having random order, no NAs (missing values) and particular cardinality factor (group size question 1 `k=100`). To see timings for other cases go to the very bottom of this page. If a solution is missing on particular data size timings table refer to benchplot for reasons and check its speed on smaller data size tab.  
...
pretty_component = function(x) gsub("_", " ", gsub("cpu", "CPU", gsub("gpu", "GPU", gsub("gb", "GB", x, fixed=TRUE), fixed=TRUE), fixed=TRUE), fixed=TRUE)
as.data.table(na.omit(fread("nodenames.csv")[lld_nodename, on="nodename", t(.SD)]), keep.rownames=TRUE)[rn!="nodename", .(Component=pretty_component(rn), Value=V1)][, kk(.SD)]
```
...
lld[, .(task, in_rows, data, knasorted, question_group)
    ][, unique(.SD)
      ][, .(benchplot=link(data, question_group, task)), .(task, `in rows`=in_rows, `data description`=knasorted)
        ][, kk(.SD)]
```
