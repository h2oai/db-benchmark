...
To access all timings data download [time.csv](https://h2oai.github.io/db-benchmark/time.csv) and use:
```r
fread("time.csv")
```
For most recent timings use:
```r
fread("time.csv")[order(timestamp), .SD[.N], by=.(task, data, in_rows, question, solution, fun, run, cache)]
```
