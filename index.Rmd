...
plot_all = function(dt, what, subset_task) {
  # not used now due to deepayan/lattice#7
...
png(sprintf("%s-%s.png", subset_task="sort", what="cpu"), width=1080, height=720)
lattice::barchart(
    time_sec ~ run |
      factor(question, levels=unique(question)) +
      factor(in_rows, levels=lvls<-rev(as.character(unique(in_rows))),
           labels=sapply(seq_along(lvls), function(i) pretty_sci(lvls[i]))),
    data = dt, groups = paste(solution, version), ylim=c(0, NA),
    horizontal=FALSE, subset = task=="sort", auto.key=TRUE, scales=list(relation="free"),
    main = paste("sort", "time elapsed - as of", paste(format(as.POSIXct(dt[, unique(range(batch))], origin="1970-01-01"), "%Y-%m-%d"), collapse="~")), xlab = "run", ylab = "seconds"
)
dev.off()
png(sprintf("%s-%s.png", subset_task="sort", what="mem"), width=1080, height=720)
lattice::barchart(
  mem_gb ~ run |
    factor(question, levels=unique(question)) +
    factor(in_rows, levels=lvls<-rev(as.character(unique(in_rows))),
  ...
  data = dt, group = paste(solution, version), ylim=c(0, NA),
  horizontal=FALSE, subset = task=="sort", auto.key=TRUE, scales=list(relation="free"),
  main = paste("sort", "RSS memory usage - as of", paste(format(as.POSIXct(dt[, unique(range(batch))], origin="1970-01-01"), "%Y-%m-%d"), collapse="~")), xlab = "run", ylab = "GB"
)
dev.off()
```
...
png(sprintf("%s-%s.png", subset_task="groupby", what="cpu"), width=1080, height=720)
lattice::barchart(
    time_sec ~ run |
      factor(question, levels=unique(question)) +
      factor(in_rows, levels=lvls<-rev(as.character(unique(in_rows))),
    ...
    main = paste("groupby", "time elapsed - as of", paste(format(as.POSIXct(dt[, unique(range(batch))], origin="1970-01-01"), "%Y-%m-%d"), collapse="~")), xlab = "run", ylab = "seconds"
)
dev.off()
png(sprintf("%s-%s.png", subset_task="groupby", what="mem"), width=1080, height=720)
lattice::barchart(
  mem_gb ~ run |
    factor(question, levels=unique(question)) +
    factor(in_rows, levels=lvls<-rev(as.character(unique(in_rows))),
  ...
  data = dt, group = paste(solution, version), ylim=c(0, NA),
  horizontal=FALSE, subset = task=="groupby", auto.key=TRUE, scales=list(relation="free"),
  main = paste("groupby", "RSS memory usage - as of", paste(format(as.POSIXct(dt[, unique(range(batch))], origin="1970-01-01"), "%Y-%m-%d"), collapse="~")), xlab = "run", ylab = "GB"
)
dev.off()
```
...
By query [_benchplot_ 1e7](grouping.1e7.png)
...
By query [_benchplot_ 1e8](grouping.1e8.png)
...
<!-- By query [_benchplot_](grouping.1e9.png) -->
