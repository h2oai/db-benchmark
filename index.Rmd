...
Main charts embedded directly in reports present timings of three runs (every command is run 3 times) for each task. Scenario of particular task is not presented in this plot, thus in case if a task has multiple scenarios their timings will be averaged. You can preview timings for different datasizes using tabs.  
...
#### Summary lattice plots

Under each task there are links to summarized statistics, one for timings and another one for memory usage. Those statistics are most detailed but least readable.  

#### Benchplot

Benchplot is a horizontal barchart for each task and scenario. It presents single datasize at a time and timings of first and second run. It also shows syntax needed to perform particular task and scenario. It is built from [Grouping benchmark](https://github.com/Rdatatable/data.table/wiki/Benchmarks-:-Grouping) scripts. Links to benchplots are currently available only for `groupby` task and can be found under each datasize tab.
...
[Detailed 1e9 benchplot](grouping.1e9.png)  
...
```{r o_ready0, echo=FALSE, include=FALSE}
plot_all(dt, "cpu", "read")
plot_all(dt, "mem", "read")
```

Detailed summary of benchmark available for [cpu](read-cpu.png) and [memory](read-mem.png)  

### 1e7

```{r o_read1, echo=FALSE}
plot_op(1e7, "read", dt)
```

### 1e8

```{r o_read2, echo=FALSE}
plot_op(1e8, "read", dt)
```

### 1e9 {.active}

```{r o_read3, echo=FALSE}
plot_op(1e9, "read", dt)
```

### 5e9

```{r o_read4, echo=FALSE, message=FALSE}
plot_op(5e9, "read", dt)
```

### 1e10

```{r o_read5, echo=FALSE, message=FALSE}
plot_op(1e10, "read", dt)
```
...
- `pandas` was failing with 1e9 rows on 125GB memory machine due to lack of memory, `groupby` benchmark was run on 244GB memory machine to produce 1e9 rows grouping statistics
...
- currently there is only inner join on integer column
- join is made on complex case data, high cardinality big to big join
...
## Read description {.tabset .tabset-fade .tabset-pills}

### Notes {.active}

- `readr` (marked as `dplyr` solution) failed to detect column types properly, types were provided to function call giving speed advantage
- `readr` (marked as `dplyr` solution) failed with 1e9 rows on 125GB memory machine due to lack of memory
- R solutions has an overhead of populating _R's global string cache_ which is at the time of writing not thread safe so has to be done using single core

### Test scripts

```{r read_source, echo=FALSE, results='asis'}
task_source("read", DT)
```
...
```{r read_data_size, echo=FALSE,}
metadata[task=="read", .N, .(rows, gb)][, .(`Rows`=rows, GB=gb)]
```

```{r read_data_head, echo=FALSE, include=FALSE}
# same as output so include=FALSE
```

example output for a `all rows` question:

```{r read_ans_head, echo=FALSE}
data.table(id1 = c("id046", "id041", "id036", "id067", "id047", 
"id003"), id2 = c("id018", "id047", "id039", "id100", "id086", 
"id002"), id3 = c("id0000006976", "id0000009477", "id0000003195", 
"id0000001035", "id0000005874", "id0000001900"), id4 = c(97L, 
89L, 23L, 44L, 79L, 10L), id5 = c(67L, 75L, 33L, 31L, 75L, 34L
), id6 = c(7103L, 699L, 7180L, 3189L, 5530L, 4013L), v1 = c(3L, 
4L, 1L, 3L, 5L, 3L), v2 = c(5L, 4L, 2L, 2L, 4L, 1L), v3 = c(2.4358, 
15.3333, 82.1465, 32.4652, 2.4358, 73.0214))
```
...
`groupby` task on 32 cores and 244GB memory
...
- modin - [recent stable](https://pypi.python.org/pypi/modin) - `r dt[solution=="modin", solution.date(solution[.N], version[.N], git[.N])]`
...
Benchmark timings are being collected over a period of `r paste(range(DT$datetime), collapse=" - ")` and stored in a single csv file. Unless stated otherwise, plots presented in this document are based on most recent benchmark timings. Those were collected over a period of `r paste(range(dt$datetime), collapse=" - ")`.  

To access all timings data you can use `fread("time.csv")` on [time.csv](https://h2oai.github.io/db-benchmark/time.csv). For most recent timings use `fread("time.csv")[order(timestamp), .SD[.N], by=.(task, data, in_rows, question, solution, fun, run, cache)]`.
