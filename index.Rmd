...
by_data = function(dt, .in_rows, .task) {
  ...
  d = rollup(wide, by=c("data","question"), j=lapply(.SD, sum), id=TRUE)
  setorderv(d, "data", na.last = TRUE)
  d[, c("rows","q1_grp.size","NA_pct","sorted") := decompose_dataname(data, .task), by=1:length(data)]
  setcolorder(d, c("data","rows","q1_grp.size","NA_pct","sorted","question"))
  d[grouping==0L, "data":=""]
