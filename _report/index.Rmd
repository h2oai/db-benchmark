...
loop_benchplot(dt_groupby2014, report_name="groupby2014", syntax.dict=groupby.syntax.dict, exceptions=groupby.exceptions, solution.dict=solution.dict, data_namev=data_name, q_groupv="basic", title.txt.fun = header_title_fun, question.txt.fun = groupby_q_title_fun, cutoff = "spark", pending = "Modin")
```
...
![](./groupby/G0_1e7_1e2_0_0_basic.png)

#### 5 GB

##### **basic questions**

![](./groupby/G0_1e8_1e2_0_0_basic.png)

#### 50 GB {.active}

##### **basic questions**

![](./groupby/G0_1e9_1e2_0_0_basic.png)
...
- For a cuDF solution _on-disk_ memory actually means main memory because this is where memory spils from a GPU memory. Note as of the current moment only single GPU is used (11 GB), see [#148](https://github.com/h2oai/db-benchmark/issues/148) for more info
- ClickHouse queries were made against `mergetree` table engine, see [#91](https://github.com/h2oai/db-benchmark/issues/91) for details. Moreover ClickHouse queries are `CREATE TABLE ans AS SELECT ...` to match the functionality provided by other solutions in terms of caching results of queries, see [#151](https://github.com/h2oai/db-benchmark/issues/151).
