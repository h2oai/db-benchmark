...
- For a cuDF solution _on-disk_ memory actually means main memory because this is where memory spils from a GPU memory. Note as of the current moment only single GPU is used (11 GB), see [#116](https://github.com/h2oai/db-benchmark/issues/116) for status.
...
- Because of the above, as of current moment, join timings of python datatable suffers from an extra deep copy, or in big-to-big join, writing answer to disk.
