...
- ClickHouse queries were made against `mergetree` table engine, see [#91](https://github.com/h2oai/db-benchmark/issues/91) for details. Moreover ClickHouse queries are `CREATE TEMPORARY TABLE ans AS SELECT ...` to match the functionality provided by other solutions in terms of caching results of queries, see [#151](https://github.com/h2oai/db-benchmark/issues/151).
...
lld[order(task), .(task, in_rows, data, knasorted, question_group)
    ][, unique(.SD)
