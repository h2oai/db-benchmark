...
plot = function(d, s, n, t) {
  ...
  fn = function(d) {
    # workaround deepayan/lattice#19
    if (uniqueN(d$batch)<2L) stop("not enough ticks on X axis") else d
  }
  tryCatch(xyplot(
    data=fn(d[task==t & solution==s & in_rows==n][, which(f) := lapply(.SD, factor), .SDcols=f]),
    time_sec~batch|data+question_group,
    groups=question,
    auto.key=list(columns=length(levels(d$question_group)), lines=TRUE, points=FALSE),
    type="l",
