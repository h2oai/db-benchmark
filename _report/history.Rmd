...
- missing summary values are likely to occur due to the missing timing values, for example, due to the performance regression a question haven't finished, then we should not skip it during calculating `mean`. As a result there might be not much to show, for example, `pandas` fails fully on 1e9 size, and fully on q10 groupby, thus aggregates over any of those dimensions will yield missing values.
