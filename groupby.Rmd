...
```{r links_plots}
link = function(x, report_name) sprintf("[%s](%s/%s.png)", x, report_name, "plots", gsub("G1_", "groupby.", x, fixed=TRUE))
```
...
&nbsp;  
Plots of all cases can be found at `r dt_task[in_rows=="1e7", paste(link(unique(data), report_name=report_name), collapse=", ")]`. Below first run timings.
...
&nbsp;  
Plots of all cases can be found at `r dt_task[in_rows=="1e8", paste(link(unique(data), report_name=report_name), collapse=", ")]`. Below first run timings.
...
&nbsp;  
Plots of all cases can be found at `r dt_task[in_rows=="1e9", paste(link(unique(data), report_name=report_name), collapse=", ")]`. Below first run timings.
