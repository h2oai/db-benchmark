...
- impala running running using lxc

## Benchmark

```{r init, echo=FALSE, message=FALSE}
library(data.table)
csv.file = Sys.getenv("CSV_TIME_FILE", "time.csv")
...
dt = fread(csv.file, na.strings=c("","NA","NaN"), sep=",", colClasses=c(batch="integer", comment="character", version="character", git="character", question="character", data="character")) # file stores multiple batches of benchmark run, so the filtering below
dt = dt[task=="join"] # keep join stats only for now
...
grain = c("task","in_rows","out_rows","solution","col","fun") # "data" for now removed from grain, as now various data used, will be addressed when running same tests on various datasets
