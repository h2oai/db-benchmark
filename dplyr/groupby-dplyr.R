...
t = system.time(print(dim(ans<-DF %>% group_by(id1) %>% summarise(v1=sum(v1)))))[["elapsed"]]
m = memory_usage()
...
t = system.time(print(dim(ans<-DF %>% group_by(id1, id2) %>% summarise(v1=sum(v1)))))[["elapsed"]]
m = memory_usage()
chkt = system.time(chk<-summarise(ungroup(ans), v1=sum(bit64::as.integer64(v1))))[["elapsed"]]
write.log(run=1L, task=task, data=data_name, in_rows=nrow(DF), question=question, out_rows=nrow(ans), out_cols=ncol(ans), solution=solution, version=ver, git=git, fun=fun, time_sec=t, mem_gb=m, cache=cache, chk=make_chk(chk), chk_time_sec=chkt)
rm(ans)
...
t = system.time(print(dim(ans<-DF %>% group_by(id3) %>% summarise(v1=sum(v1), v3=mean(v3)))))[["elapsed"]]
m = memory_usage()
...
t = system.time(print(dim(ans<-DF %>% group_by(id4) %>% summarise_at(.fun="mean", .vars=c("v1","v2","v3")))))[["elapsed"]]
m = memory_usage()
...
t = system.time(print(dim(ans<-DF %>% group_by(id6) %>% summarise_at(.funs="sum", .vars=c("v1","v2","v3")))))[["elapsed"]]
m = memory_usage()
...
t = system.time(print(dim(ans<-DF %>% group_by(id2, id4) %>% summarise(median_v3=median(v3), sd_v3=sd(v3)))))[["elapsed"]]
m = memory_usage()
...
#question = "sum v3 count by id1:id6" # q10 # on hold due to tidyverse/dplyr#4090
#t = system.time(print(dim(ans<-DF %>% group_by(id1, id2, id3, id4, id5, id6) %>% summarise(v3=sum(v3), count=n()))))[["elapsed"]]
#m = memory_usage()
