...
m = memory_usage()
t_start = timeit.default_timer()
chk = list(ans.lazy().select([pl.sum("v1_sum"), pl.sum("v3_mean")]).collect().to_numpy())
chkt = timeit.default_timer() - t_start
write_log(task=task, data=data_name, in_rows=in_rows, question=question, out_rows=ans.shape[0], out_cols=ans.shape[1], solution=solution, version=ver, git=git, fun=fun, run=1, time_sec=t, mem_gb=m, cache=cache, chk=make_chk(chk), chk_time_sec=chkt, on_disk=on_disk)
del ans
gc.collect()
...
m = memory_usage()
t_start = timeit.default_timer()
chk = list(ans.lazy().select([pl.sum("v1_mean"), pl.sum("v2_mean"), pl.sum("v3_mean")]).collect().to_numpy())
chkt = timeit.default_timer() - t_start
write_log(task=task, data=data_name, in_rows=in_rows, question=question, out_rows=ans.shape[0], out_cols=ans.shape[1], solution=solution, version=ver, git=git, fun=fun, run=1, time_sec=t, mem_gb=m, cache=cache, chk=make_chk(chk), chk_time_sec=chkt, on_disk=on_disk)
del ans
gc.collect()
...
m = memory_usage()
t_start = timeit.default_timer()
chk = list(ans.lazy().select([pl.sum("v1_sum"), pl.sum("v3_sum"), pl.sum("v3_sum")]).collect().to_numpy())
chkt = timeit.default_timer() - t_start
write_log(task=task, data=data_name, in_rows=in_rows, question=question, out_rows=ans.shape[0], out_cols=ans.shape[1], solution=solution, version=ver, git=git, fun=fun, run=1, time_sec=t, mem_gb=m, cache=cache, chk=make_chk(chk), chk_time_sec=chkt, on_disk=on_disk)
del ans
gc.collect()
...
t_start = timeit.default_timer()
print(ans)
chk = list(ans.lazy().select([pl.sum("v3_median"), pl.sum("v3_std")]).collect().to_numpy())
chkt = timeit.default_timer() - t_start
write_log(task=task, data=data_name, in_rows=in_rows, question=question, out_rows=ans.shape[0], out_cols=ans.shape[1], solution=solution, version=ver, git=git, fun=fun, run=1, time_sec=t, mem_gb=m, cache=cache, chk=make_chk(chk), chk_time_sec=chkt, on_disk=on_disk)
del ans
gc.collect()
