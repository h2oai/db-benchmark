...
with pl.StringCache():
    ...
    x = x.with_columns([x["id1"].cast(pl.Categorical), 
                        x["id2"].cast(pl.Categorical),
                        x["id3"].cast(pl.Categorical)])
...
gc.collect()
t_start = timeit.default_timer()
ans = x.groupby(["id4","id5"]).agg([pl.median("v3").alias("median_v3"), pl.std("v3").alias("std_v3")]).collect()
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
chk = ans.lazy().select([pl.col("median_v3").sum(), pl.col("std_v3").sum()]).collect().to_numpy()[0]
chkt = timeit.default_timer() - t_start
write_log(task=task, data=data_name, in_rows=in_rows, question=question, out_rows=ans.shape[0], out_cols=ans.shape[1], solution=solution, version=ver, git=git, fun=fun, run=2, time_sec=t, mem_gb=m, cache=cache, chk=make_chk(chk), chk_time_sec=chkt, on_disk=on_disk)
print(ans.head(3), flush=True)
print(ans.tail(3), flush=True)
...
gc.collect()
t_start = timeit.default_timer()
ans = x.drop_nulls("v3").sort("v3", descending=True).groupby("id6").agg(col("v3").head(2).alias("largest2_v3")).explode("largest2_v3").collect()
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.groupby(["id2","id4"]).agg((pl.corr("v1","v2")**2).alias("r2")).collect()
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
